...
DARK_SEQUENTIAL = ["#FFD166", "#06D6A0", "#118AB2", "#073B4C", "#EF476F"]  # ألوان زاهية ومختلفة
DARK_EARTH = [
    [0.0, "#0D1B2A"], [0.2, "#1B263B"], [0.4, "#415A77"],
    [0.6, "#778DA9"], [0.8, "#E0E1DD"], [1.0, "#F4D35E"]
]

# ألوان متباينة للقائمة والخطوط والعناوين
FONT_HIGH = dict(family="Segoe UI", size=14, color="#F4F4F4")   # أبيض فاتح للخط
TITLE_FONT = dict(family="Segoe UI", size=20, color="#FFD166")   # أصفر مشرق للعنوان
...
def _update_layout(fig):
    fig.update_layout(
        ...
        paper_bgcolor="#0D1B2A",       # خلفية عامة غامقة
        plot_bgcolor="#1B263B",        # خلفية المخطط غامقة مختلفة قليلاً
        xaxis_gridcolor="#415A77",     # خطوط الشبكة متوسطة التباين
        yaxis_gridcolor="#415A77",
        xaxis_tickfont=FONT_HIGH,
        yaxis_tickfont=FONT_HIGH,
    )
    return fig
...
def generate_bar_plot(data, info):
    ...
    fig = px.bar(data, x=_x, y=_y, orientation='h',
                 color=_x, color_continuous_scale=DARK_EARTH,
                 text=_x, template="plotly_white")
    fig.update_traces(texttemplate='%{text:,.0f}', textposition='outside',
                      marker_line_width=0)
    fig.update_layout(title=title, xaxis_title=_x, yaxis_title=y_lab,
...
def generate_heatmap(data, info):
    switch, grp_by = list(info['grouping'].keys())[0], list(info['grouping'].values())[0]
    _y, _x = info['x'], grp_by if switch else info['y']   # _y = metric, _x = group

    title = f"{_y} across {_x} groups"
    # pivot:  rows = metric values, cols = groups
...
# ---------- 3. Line (correct dominant category + hover) ----------
def generate_line_plot(data, tokens):
    _x, _y = 'Pub_Date', tokens['_by']
    channel = tokens['channel']

    # 1. dominant category across the whole channel slice
    ...
    title = (f"{channel} – {_y} over time<br>"
             f"<sub style='font-size:14px;font-weight:300;'>"
             f"Dominant category: {category}</sub>")

    # 3. line + hover with category per point
